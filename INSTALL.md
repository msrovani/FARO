...
Para instalação 100% automatizada sem intervenção manual, execute:
...
- ✅ Executar migrations
- ✅ Criar seed data
- ✅ Iniciar todos os serviços (Server Core, Web Console, Analytics Dashboard)
- ✅ Verificar health check
- ✅ Gerar relatório final

**Após a instalação:**
- Verifique se tudo está funcionando: `.\verify-installation.ps1`
- Acesse o sistema: http://localhost:8000
- Login: admin@faro.pol / password

**Para iniciar serviços posteriormente:**
```powershell
.\start-services.ps1
```
...
**Opções avançadas:**
```powershell
# Pular Docker (usar PostgreSQL manual)
.\install-faro.ps1 -SkipDocker

# Pular Web Console
.\install-faro.ps1 -SkipWebConsole

# Pular Analytics Dashboard
.\install-faro.ps1 -SkipAnalytics

# Usar portas alternativas
.\install-faro.ps1 -ServerPort 8001 -WebConsolePort 3001
```
...
- Node.js 18.x or higher (for Web Intelligence Console - Next.js 15.x)
- Git (for cloning repository)
...
### 7. � Apply PostgreSQL Performance Tuning (Recommended for Production)

**⚠️ IMPORTANT:** This step applies performance optimizations based on best practices for PostGIS and geospatial workloads. This is optional but highly recommended for production deployments.

**🔍 Pre-check:**
```powershell
# Check if PostgreSQL configuration file exists
$pgConf = "C:\Program Files\PostgreSQL\16\data\postgresql.conf"
if (Test-Path $pgConf) { Write-Host "✅ postgresql.conf found" } else { Write-Host "❌ postgresql.conf not found" }
```

**For Humans:**
1. Copy the tuning configuration file:
```powershell
Copy-Item "$env:FARO_HOME\database\postgresql-tuning.conf" "C:\Program Files\PostgreSQL\16\data\postgresql-tuning.conf.bak"
```

2. Review and adjust memory settings in `database/postgresql-tuning.conf` based on your server's available RAM:
   - `shared_buffers`: 25% of RAM (e.g., 2GB for 8GB RAM)
   - `effective_cache_size`: 75% of RAM (e.g., 6GB for 8GB RAM)
   - `work_mem`: Adjust based on formula in file

3. Apply the configuration to PostgreSQL:
```powershell
# Backup current configuration
Copy-Item "C:\Program Files\PostgreSQL\16\data\postgresql.conf" "C:\Program Files\PostgreSQL\16\data\postgresql.conf.backup"

# Apply tuning settings (manual edit required)
# Open postgresql.conf and add settings from database/postgresql-tuning.conf
...
# Note: Automatic editing of postgresql.conf requires careful parsing
# For safety, this step requires manual review in production
Write-Host "⚠️  PostgreSQL tuning requires manual review of database/postgresql-tuning.conf"
Write-Host "Please adjust memory settings based on available RAM and apply to postgresql.conf"
```
...
$service = Get-Service -Name "postgresql-x64-16" -ErrorAction SilentlyContinue
if ($service.Status -eq "Running") { Write-Host "✅ PostgreSQL restarted successfully" } else { Write-Host "❌ PostgreSQL restart failed" }
```

**🔄 Workaround:** If tuning is too complex for your environment, skip this step. PostgreSQL will use default settings (reduced performance but functional).

---

### 8. 🔗 Configure Alembic Connection String
...
- **Migration 0020: pg_stat_statements extension** (NEW - enables query performance monitoring)

**Note:** Migrations 0010, 0011, and 0020 will gracefully skip if extensions are not available.
...
### 11. 🐳 Install Docker Desktop OR Configure Manual Extensions
...
### 12. 💾 Install MinIO for Asset Storage (Optional)
...
### 13. ⏱️ Install TimescaleDB Extension (Optional)
...
### 14. 🔄 Install Citus Extension (Optional - Skip if Docker unavailable)
...
### 15. 🔍 Automatic Detection and Configuration
...
### 16. 🎱 Configure PgBouncer (Recommended with Optimizations)

**Required Version:** PgBouncer 1.21.x or higher

**⚠️ NEW:** PgBouncer configuration has been optimized with best practices for production:
- server_reset_query = DISCARD ALL (cleans state between connections)
- server_check_delay = 30 (health checks)
- server_reset_query_always = 0 (enables prepared statements)
- tcp_keepalive settings (maintains connections on unreliable networks)
- reserve_pool configuration (handles load spikes)
...
3. The `pgbouncer-faro.ini` configuration file already includes production optimizations:
```ini
[databases]
...
### 17. 📊 Configure Prometheus (Recommended)
...
### 18. 🚀 Start FARO Server

**⚠️ NEW:** Two startup options are available:
- **Development:** Uvicorn with auto-reload (default for development)
- **Production:** Gunicorn with Uvicorn workers (recommended for production)

**Option A: Development (Uvicorn with auto-reload)**
...
**Option B: Production (Gunicorn with Uvicorn workers - NEW)**

**⚠️ RECOMMENDED FOR PRODUCTION:** Gunicorn provides better performance and stability with worker management.
...
**Gunicorn Configuration (from gunicorn_start.py):**
- Auto-detection of CPU cores for worker count
- max_requests + max_requests_jitter (prevents memory leaks)
- worker_connections = 1000 (high concurrency)
- preload app (copy-on-write optimization)
- Graceful timeout for safe restarts

**Expected Output (Development - Uvicorn):**
```
...
**Expected Output (Production - Gunicorn):**
```
...
### 20. 🌐 Verify Web Interface (Intelligence Console)
...
### 21. 📱 Verify Mobile Agent Connection
...
### 22. 🔬 Complete System Verification
...
### 25. 🔧 Redis Cache Manager (NEW - Optional Integration)

**⚠️ NEW COMPONENT:** A production-ready cache manager with cache-aside pattern has been implemented to improve performance. This is optional but recommended for production deployments.

**Purpose:**
- Reduce database load through intelligent caching
- Cache-aside pattern (lazy loading) for read-heavy workloads
- Configurable TTLs per data type (short/medium/long)
- Graceful degradation if Redis is unavailable
- SCAN-based pattern invalidation (prevents blocking)

**🔍 Pre-check:**
```powershell
# Check if Redis cache manager module exists
$cacheManager = "$env:FARO_HOME\server-core\app\utils\cache_manager.py"
if (Test-Path $cacheManager) { Write-Host "✅ Redis cache manager found" } else { Write-Host "❌ Redis cache manager not found" }
```

**Integration Steps:**

**For Humans:**
1. Import the cache manager in your services:
```python
from app.utils.cache_manager import cache_manager, cached
```

2. Use cache manager directly:
```python
...
3. Or use the decorator for automatic caching:
```python
...
**For LLM:**
The cache manager is ready to use. No installation required - it's part of the server-core codebase.

**TTL Strategy:**
- **short (60s):** Dados mutáveis (status, contadores)
- **medium (300s):** Dados normais (perfis, configurações)
- **long (3600s):** Dados estáticos (lookup tables, referências)

**🔄 Workaround:** If Redis is not available, the cache manager will gracefully degrade and bypass caching, continuing to fetch from the database.

---

## Web Services Startup and Verification

### 23. 🌐 Start Web Intelligence Console
