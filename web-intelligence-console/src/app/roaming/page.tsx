...
export default function RoamingPage() {
  ...
  const loadData = useCallback(async (plate?: string) => {
    try {
      setLoading(true);
      setError(null);
      const data = await intelligenceApi.listRoaming(plate || undefined);
      setItems(data);
    } catch (err) {
      console.error(err);
      setError("Falha ao carregar eventos de roaming/loitering.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <ConsoleShell
      title="Modulo de Roaming / Loitering"
      subtitle="Monitora aproximacao repetida, permanencia incomum e padrao pendular por placa e area."
    >
      {error ? (
      ...
      <div className="mb-6 flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Aplicar por placa para leitura mais precisa do comportamento.</p>
        </div>
        <form
      ...
      <AlgorithmEventList
        title="Eventos de roaming/loitering"
        subtitle="Consolida reincidencia em area, com risco de falso positivo explicito por evento."
        items={items}
        loading={loading}
      />
    </ConsoleShell>
