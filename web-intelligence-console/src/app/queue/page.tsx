...
import ObservationFeedback from "@/app/components/ObservationFeedback";
...
function QueueContent() {
  ...
  // Polling para atualização em tempo real da fila
  useEffect(() => {
  ...
  }, [plateFilter, selectedId, loadQueue]);
  ...
  async function submitReview(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedbackMessage("");
      // Recarrega a fila sem manter o item atual (ele deve sair da lista após análise)
      await loadQueue({ plate: plateFilter, preferredId: null });
      setSelectedObservation(null);
    } catch (err) {
      console.error(err);
  ...
  const summary = useMemo(() => {
    // Debug: mostrar valores únicos de urgency e score para verificação
    const urgencies = items.map((item) => item.urgency);
    const scores = items.map((item) => item.score_value);
    console.log("Urgencies:", [...new Set(urgencies)]);
    console.log("Scores:", scores.filter((s) => s !== null && s !== undefined));
    ...
    const scored = items.filter((item) => typeof item.score_value === "number" && item.score_value >= 60).length;
    
    console.log("Summary calculated:", { approach, intelligence, monitor, scored, total: items.length });
  ...
  return (
    ...
    >
      ...
      <section className="mb-6 grid gap-4 md:grid-cols-6">
        <SummaryCard label="Total na fila" value={items.length} />
        <SummaryCard label="Score >= 60" value={summary.scored} tone="critical" />
        <SummaryCard label="Abordagem" value={summary.approach} tone="warning" />
        <SummaryCard label="Inteligência" value={summary.intelligence} tone="info" />
        <SummaryCard label="Monitoramento" value={summary.monitor} tone="success" />
        <SummaryCard label="Sem score" value={items.filter((item) => !item.score_value).length} tone="default" />
      </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {!selectedId ? (
            ...
            <>
              <div className="rounded-2xl bg-slate-950 p-5 text-white">
                ...
                <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                  <InfoRow label="Latitude" value={selectedObservation.location.latitude.toFixed(3)} />
                  <InfoRow label="Longitude" value={selectedObservation.location.longitude.toFixed(3)} />
                  ...
                  />
                  <InfoRow label="Estado" value={selectedObservation.plate_state || "N/A"} />
                  <InfoRow label="Pais" value={selectedObservation.plate_country} />
                  <InfoRow label="Tipo Veiculo" value={selectedObservation.vehicle_type || "N/A"} />
                  <InfoRow label="Cor" value={selectedObservation.vehicle_color || "N/A"} />
                  <InfoRow label="Modelo" value={selectedObservation.vehicle_model || "N/A"} />
                  <InfoRow label="Ano" value={selectedObservation.vehicle_year?.toString() || "N/A"} />
                  <InfoRow label="Rumo" value={selectedObservation.heading ? `${selectedObservation.heading.toFixed(3)}°` : "N/A"} />
                  <InfoRow label="Velocidade" value={selectedObservation.speed ? `${selectedObservation.speed.toFixed(1)} km/h` : "N/A"} />
                  <InfoRow label="Sync Status" value={selectedObservation.sync_status} />
                  <InfoRow label="Device ID" value={selectedObservation.device_id.slice(0, 8) + "..."} />
                </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-100 bg-white sm:grid-cols-4">
                  <div className="flex items-center gap-2 p-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-medium text-slate-900">{selectedObservation.heading ? `${selectedObservation.heading.toFixed(3)}°` : "N/D"}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 p-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-medium text-slate-900">{selectedObservation.speed ? `${selectedObservation.speed.toFixed(1)} km/h` : "N/D"}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 p-3">
              ...
              <form id="review-form" className="mt-4 space-y-4" onSubmit={submitReview}>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-medium text-slate-700">Destinatário</label>
                    <select
                    ...
                    >
                      ...
                      <option value={selectedObservation.agent_id}>Agente: {selectedObservation.agent_name}</option>
                    </select>
                  </div>
                  <div>
                    <label className="mb-2 block text-sm font-medium text-slate-700">Nível de sensibilidade</label>
                    <select
            ...
            {selectedObservation && (
              <div className="mt-4">
                <ObservationFeedback 
                  observationId={selectedObservation.observation_id}
                  plateNumber={selectedObservation.plate_number}
                />
              </div>
            )}
          </section>
