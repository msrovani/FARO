...
export default function AlertsPage() {
  ...
  const handleAcknowledgeAlert = async (alertId: string) => {
    try {
      ...
      setAlerts(alerts.map(a => 
        a.plate_number === alertId ? { ...a, requires_review: false } : a
      ));
    } catch (err) {
      console.error(err);
        ...
        {selectedAlert && (
          <div className="absolute bottom-4 right-4 bg-gray-800 rounded-lg p-4 shadow-xl w-80">
            ...
            <div className="pt-3 border-t border-gray-700 mt-3 flex gap-2">
              <button
                onClick={() => handleAcknowledgeAlert(selectedAlert.plate_number)}
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded text-xs"
              >
