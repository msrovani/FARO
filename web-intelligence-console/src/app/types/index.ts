...
export interface AgentMovementAnalysisResult {
  summaries: AgentMovementSummary[];
  total_agents: number;
...
export interface AgentObservationCorrelation {
  agent_id: string;
  agent_name: string;
  total_observations: number;
  total_agent_locations: number;
  observations_near_agent: number;
  correlation_rate: number;
  average_distance_to_observations: number;
  most_productive_areas: Array<{
    grid_key: string;
    count: number;
  }>;
  peak_detection_hours: number[];
}

export interface TacticalPositioningRecommendation {
