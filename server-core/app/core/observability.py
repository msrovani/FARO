...
def setup_metrics(app: FastAPI) -> None:
    ...
    async def metrics_json(
        ...
        try:
            ...
            from app.db.base import VehicleObservation, SuspicionReport, Alert, IntelligenceReview, WatchlistEntry
            from datetime import datetime, timedelta
            
            now = datetime.utcnow()
            today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
            
            # Count observations today
            obs_today = await db.scalar(
                select(func.count(VehicleObservation.id)).where(
                    VehicleObservation.created_at >= today_start
                )
            ) or 0
            metrics_data['observations_today'] = obs_today
            
            # Count suspicion reports by level (severity)
            # SuspicionLevel enum has: low, medium, high
            susp_high = await db.scalar(
                select(func.count(SuspicionReport.id)).where(
                    and_(SuspicionReport.level == 'high', SuspicionReport.created_at >= today_start)
                )
            ) or 0
            susp_medium = await db.scalar(
                select(func.count(SuspicionReport.id)).where(
                    and_(SuspicionReport.level == 'medium', SuspicionReport.created_at >= today_start)
                )
            ) or 0
            susp_low = await db.scalar(
                select(func.count(SuspicionReport.id)).where(
                    and_(SuspicionReport.level == 'low', SuspicionReport.created_at >= today_start)
                )
            ) or 0
            ...
            # Count intelligence reviews (confirmed vs rejected)
            review_confirmed = await db.scalar(
                select(func.count(IntelligenceReview.id)).where(
                    and_(IntelligenceReview.status == 'confirmed', IntelligenceReview.created_at >= today_start)
                )
            ) or 0
            review_rejected = await db.scalar(
                select(func.count(IntelligenceReview.id)).where(
                    and_(IntelligenceReview.status == 'discarded', IntelligenceReview.created_at >= today_start)
                )
            ) or 0
