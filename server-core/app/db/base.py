...
class Device(Base):
    ...
    last_justification: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class AgentLocationLog(Base):
    ...
    __table_args__ = (
        ...
        # GIST index for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_agent_location_gist", "location", postgresql_using='gist'),
    )
...
class VehicleObservation(Base):
    ...
    __table_args__ = (
        ...
        # Índices que dependem de enums serão criados em migrations separadas
        # para evitar erros quando os enums ainda não existem
    )
...
class Alert(Base):
    ...
    rule: Mapped[Optional["AlertRule"]] = relationship(
        foreign_keys=[triggered_by_rule_id]
    )
...
class AnalystGeofence(Base):
    ...
    __table_args__ = (
        ...
        # GIST index for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_analyst_geofence_area_gist", "area", postgresql_using='gist'),
    )
...
class WatchlistEntry(Base):
    ...
    requires_approach: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    approach_guidance: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class RoutePattern(Base):
    ...
    __table_args__ = (
        ...
        # GIST indexes for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_route_pattern_centroid_gist", "centroid_location", postgresql_using='gist'),
        Index("ix_route_pattern_bbox_gist", "bounding_box", postgresql_using='gist'),
        Index("ix_route_pattern_corridor_gist", "corridor", postgresql_using='gist'),
        # GIN index for ARRAY columns (common_hours, common_days)
        Index("ix_route_hours_gin", "common_hours", postgresql_using='gin'),
        Index("ix_route_days_gin", "common_days", postgresql_using='gin'),
    )
...
class RouteRegionOfInterest(Base):
    ...
    __table_args__ = (
        # GIST index for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_route_region_geom_gist", "geometry", postgresql_using='gist'),
    )
...
class SensitiveAssetZone(Base):
    ...
    __table_args__ = (
        # GIST index for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_sensitive_asset_geom_gist", "geometry", postgresql_using='gist'),
    )
...
class ConvoyEvent(Base):
    ...
    __table_args__ = (
        ...
        # GIN indexes for ARRAY columns (common_hours, common_days)
        Index("ix_convoy_hours_gin", "common_hours", postgresql_using='gin'),
        Index("ix_convoy_days_gin", "common_days", postgresql_using='gin'),
    )
...
class RoamingEvent(Base):
    ...
    __table_args__ = (
        ...
        # GIST index for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_roaming_area_geom_gist", "area_geometry", postgresql_using='gist'),
        # GIN indexes for ARRAY columns (common_hours, common_days)
        Index("ix_roaming_hours_gin", "common_hours", postgresql_using='gin'),
        Index("ix_roaming_days_gin", "common_days", postgresql_using='gin'),
    )
...
class SuspiciousRoute(Base):
    ...
    __table_args__ = (
        ...
        # GIST index for PostGIS spatial queries (CRITICAL for performance)
        Index("ix_suspicious_route_geom_gist", "route_geometry", postgresql_using='gist'),
    )
...
class InterceptEvent(Base):
    ...
    # Combined intercept score
    intercept_score: Mapped[float] = mapped_column(Float, nullable=False, index=True)
    
    # Algorithm triggers that contributed to intercept decision
    watchlist_trigger: Mapped[bool] = mapped_column(Boolean, default=False)
    route_anomaly_trigger: Mapped[bool] = mapped_column(Boolean, default=False)
    impossible_travel_trigger: Mapped[bool] = mapped_column(Boolean, default=False)
    sensitive_zone_trigger: Mapped[bool] = mapped_column(Boolean, default=False)
    convoy_trigger: Mapped[bool] = mapped_column(Boolean, default=False)
    roaming_trigger: Mapped[bool] = mapped_column(Boolean, default=False)
    
    # Individual algorithm scores
    watchlist_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    route_anomaly_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    impossible_travel_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    sensitive_zone_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    convoy_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    roaming_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    recommendation: Mapped[str] = mapped_column(String(50), nullable=False)  # "APPROACH", "MONITOR", "IGNORE"
    priority_level: Mapped[str] = mapped_column(String(20), nullable=False, default="medium")  # "high", "medium", "low"
    
    # Time-based factors
    time_of_day_risk: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # Risk based on time
    day_of_week_risk: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # Risk based on day
    
    # Geographic context
    nearby_critical_assets: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Count of nearby assets
    proximity_sensitive_zone: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
    ...
    __table_args__ = (
        Index("ix_intercept_observation", "observation_id"),
        Index("ix_intercept_score", "intercept_score"),
        Index("ix_intercept_recommendation", "recommendation"),
        Index("ix_intercept_created", "created_at"),
    )
...
class BoletimAtendimento(Base):
    ...
    transmission_status: Mapped[str] = mapped_column(
        String(20),
        Enum(BATransmissionStatusEnum, name="batransmissionstatus", create_constraint=True),
        nullable=False,
        default=BATransmissionStatusEnum.PENDING.value,
        index=True,
    )
...
# =============================================================================
# ALERT RULES — Configuração Dinâmica de Regras de Alerta
# =============================================================================
...
class AlertRuleSeverityEnum(str, PyEnum):
...
class AlertRule(Base):
    ...
    Permite criar regras dinâmicas para geração de alertas baseadas em:
    - Tipo de alerta (watchlist, rota suspeita, hotspot, etc.)
    - Condições personalizadas (JSON)
    - Severidade mínima
    - Agência específica ou global
    ...
    rule_type: Mapped[AlertRuleTypeEnum] = mapped_column(
        Enum(AlertRuleTypeEnum, name="alertruletype", create_constraint=True),
        nullable=False,
        index=True,
    )
    
    # Condições da regra em JSON (flexível para diferentes tipos de regras)
    conditions: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=False, default=dict)
    
    severity: Mapped[AlertRuleSeverityEnum] = mapped_column(
        Enum(AlertRuleSeverityEnum, name="alertruleseverity", create_constraint=True),
        nullable=False,
        default=AlertRuleSeverityEnum.MEDIUM,
    )
    
    # Se a regra está ativa
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, index=True)
    
    # Agência que criou a regra (null = regra global/admin)
    ...
    # Prioridade de execução (regras com maior prioridade são executadas primeiro)
