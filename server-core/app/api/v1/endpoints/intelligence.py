...
from app.api.v1.deps import get_db, require_intelligence_analyst
...
async def intelligence_root():
    """Intelligence API root endpoint."""
    return {
        "module": "F.A.R.O. Intelligence API",
        "version": "1.0.0",
        "description": "API para console de inteligência - casos, observações, feedback, análises",
        "endpoints": [
            "/observations",
            "/cases",
            "/feedback",
            "/analytics/overview",
            "/analytics/suspicion-recurrence",
            "/watchlist",
            "/suspicious-routes",
            "/hotspots",
            "/route-prediction",
...
async def build_hierarchy_filter_condition(current_user: User, db: AsyncSession, column):
    """Build a SQLAlchemy filter condition for agency hierarchy."""
    from app.services.agency_hierarchy_service import build_hierarchy_filter_condition as build_filter
    
    return await build_filter(
        db,
...
def get_agency_scope_filter(current_user: User, column):
    ...
    # Use hierarchy service for filtering
    from app.services.agency_hierarchy_service import build_hierarchy_filter_condition
    
    hierarchy_filter = build_hierarchy_filter_condition(
    ...
    # Fallback to simple agency filter
...
async def build_algorithm_results_for_observation(
    ...
    watchlist_hits, impossible_events, route_events, sensitive_events, convoy_events, roaming_events = await asyncio.gather(
        db.execute(select(WatchlistHit).where(WatchlistHit.observation_id == observation_id)),
        db.execute(select(ImpossibleTravelEvent).where(ImpossibleTravelEvent.observation_id == observation_id)),
        db.execute(select(RouteAnomalyEvent).where(RouteAnomalyEvent.observation_id == observation_id)),
        db.execute(
            select(SensitiveAssetRecurrenceEvent).where(
                SensitiveAssetRecurrenceEvent.observation_id == observation_id
            )
        ),
        db.execute(select(ConvoyEvent).where(ConvoyEvent.observation_id == observation_id)),
        db.execute(select(RoamingEvent).where(RoamingEvent.observation_id == observation_id)),
    )

    # Process watchlist hits
    for hit in watchlist_hits.scalars().all():
        results.append(
    ...
    # Process impossible travel events
    for event in impossible_events.scalars().all():
        results.append(
            serialize_algorithm_result(
                event_id=event.id,
    ...
    # Process route anomaly events
    for event in route_events.scalars().all():
        results.append(
            serialize_algorithm_result(
                event_id=event.id,
    ...
    # Process sensitive asset recurrence events
    for event in sensitive_events.scalars().all():
        results.append(
            serialize_algorithm_result(
                event_id=event.id,
    ...
    # Process convoy events
    for event in convoy_events.scalars().all():
        results.append(
            serialize_algorithm_result(
                event_id=event.id,
    ...
    for event in roaming_events.scalars().all():
        results.append(
            serialize_algorithm_result(
                event_id=event.id,
...
async def get_intelligence_queue(
    ...
    if settings.queue_auto_prioritization_enabled:
        ...
        query = (
            query.outerjoin(SuspicionScore, and_(
                SuspicionScore.observation_id == VehicleObservation.id,
                SuspicionScore.observation_id == SuspicionScore.observation_id  # Explicit JOIN condition
            ))
            .order_by(
                case(
...
async def get_analytics_overview(
    ...
    critical_scores = (
        await db.execute(
            select(func.count(SuspicionScore.id)).where(SuspicionScore.final_label == "critical_match")
        )
    ).scalar() or 0
    watchlist_hits = (
        await db.execute(
    ...
    high_scores = (
        await db.execute(
            select(func.count(SuspicionScore.id)).where(SuspicionScore.final_label == "relevant_match")
        )
    ).scalar() or 0
    moderate_scores = (
        await db.execute(
            select(func.count(SuspicionScore.id)).where(SuspicionScore.final_label == "weak_match")
        )
    ).scalar() or 0
    low_scores = (
        await db.execute(
            select(func.count(SuspicionScore.id)).where(SuspicionScore.final_label == "no_match")
        )
    ).scalar() or 0
    
    # Algorithm runs count
    from app.db.base import AlgorithmRun
    algorithm_runs = (
        await db.execute(
            select(func.count(AlgorithmRun.id)).where(
                AlgorithmRun.created_at >= last_30_days,
                agency_filter if hasattr(AlgorithmRun, 'agency_id') else true()
            )
        )
    ).scalar() or 0
    
    # OCR success rate (different from correction rate)
    ocr_success_rate = (
        await db.execute(
            select(
                func.avg(
                    case(
                        (PlateRead.ocr_confidence >= 0.7, 1.0),
                        else_=0.0,
                    )
                )
            )
...
async def list_agencies(
    ...
    if current_user.role != UserRole.ADMIN:
        ...
        from app.services.agency_hierarchy_service import get_agency_hierarchy_ids
        
        hierarchy_ids = await get_agency_hierarchy_ids(
            db,
            current_user.agency_id,
            include_children=True
        )
        query = query.where(Agency.id.in_(hierarchy_ids))
...
async def get_intercept_events(
    ...
    """
    Get INTERCEPT algorithm events for approach recommendations.
    ...
    """
    from app.db.base import InterceptEvent
    
    # Build query with agency filter
    query = select(InterceptEvent).join(VehicleObservation, VehicleObservation.id == InterceptEvent.observation_id)
    ...
    if current_user.role == UserRole.ADMIN:
        ...
        # Other users see only their agency
        query = query.where(VehicleObservation.agency_id == current_user.agency_id)
    
    # Apply filters
    if recommendation:
        query = query.where(InterceptEvent.recommendation == recommendation.upper())
    
    if priority_level:
        query = query.where(InterceptEvent.priority_level == priority_level.lower())
    
    # Order by intercept score (highest first) and limit
    query = query.order_by(InterceptEvent.intercept_score.desc()).limit(limit)
    ...
    return [
        {
            ...
            "plate_number": event.observation.plate_number if hasattr(event, 'observation') else "unknown",
            "intercept_score": event.intercept_score,
            ...
            "false_positive_risk": event.false_positive_risk,
            "triggers": {
                "watchlist": event.watchlist_trigger,
                "route_anomaly": event.route_anomaly_trigger,
                "impossible_travel": event.impossible_travel_trigger,
                "sensitive_zone": event.sensitive_zone_trigger,
                "convoy": event.convoy_trigger,
                "roaming": event.roaming_trigger,
            },
            "individual_scores": {
                "watchlist": event.watchlist_score,
                "route_anomaly": event.route_anomaly_score,
                "impossible_travel": event.impossible_travel_score,
                "sensitive_zone": event.sensitive_zone_score,
                "convoy": event.convoy_score,
                "roaming": event.roaming_score,
            },
            "time_factors": {
                "time_of_day_risk": event.time_of_day_risk,
                "day_of_week_risk": event.day_of_week_risk,
            },
            "geographic_context": {
                "nearby_critical_assets": event.nearby_critical_assets,
                "proximity_sensitive_zone": event.proximity_sensitive_zone,
            },
