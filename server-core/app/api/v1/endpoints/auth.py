...
async def login(
    ...
    try:
        ...
        import traceback
        logger.error(f"Error during login query: {e}")
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(
...
async def list_users(
    ...
    if current_user.role == UserRole.ADMIN:
        ...
        hierarchy_filter = build_hierarchy_filter_condition(
            ...
            column=User.agency_id
        )
        
        if hierarchy_filter is not None and hierarchy_filter is not True:
            query = query.where(hierarchy_filter)
        else:
