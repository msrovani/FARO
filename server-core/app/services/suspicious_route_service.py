...
async def check_route_match(
    ...
    SANITIZED: Uses bind parameters to prevent SQL injection.
    """
    ...
    obs_lat = match_request.location.latitude
    obs_lng = match_request.location.longitude
    
    # Single query to find all matching routes
    # Uses ST_MakePoint with bind parameters to prevent SQL injection
    query = text("""
        SELECT 
            id, name, crime_type, risk_level,
            CASE 
                WHEN ST_Intersects(route_geometry, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326)) THEN 'intersection'
                WHEN buffer_distance_meters IS NOT NULL 
                     AND ST_DWithin(route_geometry, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326), buffer_distance_meters) 
                THEN 'proximity'
                ELSE NULL
            END as match_type,
            CASE 
                WHEN buffer_distance_meters IS NOT NULL 
                     AND ST_DWithin(route_geometry, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326), buffer_distance_meters) 
                THEN ST_Distance(route_geometry, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326))
                ELSE NULL
        ...
        WHERE 
            ...
            AND (
                ST_Intersects(route_geometry, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326))
