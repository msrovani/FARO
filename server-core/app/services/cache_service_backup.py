...
cache_service = CacheService()
        ...
        """Get cached watchlist for agency."""
        cache_key = f"watchlist:agency:{agency_id}"
        if status:
            cache_key = f"{cache_key}:status:{status}"
    ...
    async def cache_watchlist(
        ...
        """Cache watchlist for agency."""
        cache_key = f"watchlist:agency:{agency_id}"
        if status:
            cache_key = f"{cache_key}:status:{status}"
        
        return await self.set(cache_key, watchlist, ttl=timedelta(minutes=10))
    
    async def invalidate_watchlist(self, agency_id: str) -> int:
        """Invalidate all watchlist caches for agency."""
        pattern = f"watchlist:agency:{agency_id}:*"
        return await self.invalidate_pattern(pattern)
    ...
    async def cache_analytics(
        ...
        return await self.set(cache_key, analytics, ttl=timedelta(minutes=1))
    
    async def invalidate_analytics(self, agency_id: Optional[str] = None) -> int:
        """Invalidate analytics cache."""
        if agency_id:
            pattern = f"analytics:*:agency:{agency_id}"
        else:
            pattern = "analytics:*"
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_keys": int(info.get("keyspace_hits", 0) + info.get("keyspace_misses", 0)),
...
async def get_cache_service() -> CacheService:
    ...
    if not cache_service.redis and cache_service.enabled:
