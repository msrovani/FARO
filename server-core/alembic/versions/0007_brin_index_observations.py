...
def upgrade() -> None:
    ...
    # BRIN indexes are 10x smaller than B-Tree for time-ordered data
    op.execute(
        sa.text(
            """
            CREATE INDEX IF NOT EXISTS ix_vehicleobservation_observed_at_local_brin
    ...
    op.execute(
        sa.text(
            """
            CREATE INDEX IF NOT EXISTS ix_vehicleobservation_created_at_brin
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS ix_vehicleobservation_created_at_brin")
    op.execute("DROP INDEX IF EXISTS ix_vehicleobservation_observed_at_local_brin")
